...
book = xlrd.open_workbook('车位信息2.xlsx')
...
for i in range (2, rows):
    name = trim(sheet.cell (i,2).value)
    phone = trim(str(sheet.cell (i,3).value).split(".")[0])
    car_license = trim(sheet.cell (i,1).value)
    carport_site = trim(str(sheet.cell (i,0).value).split(".")[0])
    try:
